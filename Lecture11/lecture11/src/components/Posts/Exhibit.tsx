...
import { ExhibitPropsI } from "@/Interfaces/ExhibitPropsI";

const Exhibit: React.FC<ExhibitPropsI> = ({ exhibit }) => {
  ...
  const { run: loadComments, loading: commentsLoading, error } = useRequest(
    () => CommentsActions.comments(exhibit.id),
    {
  ...
  const { run: removeExhibit, loading: removeLoading } = useRequest(
    () => ExhibitsActions.deleteExhibit(exhibit.id),
    {
      manual: true,
  ...
  return (
    <Card key={exhibit.id} sx={{ boxShadow: 3, maxWidth: 800, display: 'block', mx: 'auto', mb: 3 }}>
      ...
      <CardMedia
        component="img"
        image={`${API_BASE_URL}${exhibit.imageUrl}`}
        alt="Exhibit image"
        sx={{
          width: "100%",
          objectFit: "cover",
